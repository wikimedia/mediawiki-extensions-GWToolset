...
* Té el format correcte el fitxer XML? Proveu això $1.', # Fuzzy
	...
	'gwtoolset-step-1-instructions-2' => 'En aquest pas pugeu un nou fitxer de metadades al wiki. L\'eina intentarà extreure els camps de metadades disponibles en el fitxer de metadades, que llavors s\'aplicaran a una plantilla MediaWiki a "{{int:gwtoolset-step-2-heading}}".', # Fuzzy
	'gwtoolset-step-1-instructions-3' => "Si el vostre domini de fitxers multimèdia no es troba a la llista a continuació, [$1 sol·liciteu] que sigui afegit a la llista blanca de dominis a Wikimedia Commons. Aquesta llista es comprova abans d'obtenir els fitxers multimèdia. Si el vostre domini no està en aquesta llista, Wikimedia Commons no baixarà els fitxers multimèdia d'aquell domini. El millor exemple que podeu presentar en la vostra sol·licitud és un enllaç a un dels vostres fitxers multimèdia.",
	...
	'gwtoolset-mapping-media-file-url-bad' => 'No s\'ha pogut avaluar la URL del fitxer multimèdia. La URL proveeix el contingut en un format que encara no està suportat per l\'extensió. L\'URL proveïda és "$1".', # Fuzzy
...
$2',
	...
	'gwtoolset-step-1-instructions-3' => 'Falls deine Mediendateidomain unten nicht aufgelistet ist, stelle bitte eine [$1 Anfrage], dass deine Mediendateidomain zur Wikimedia-Commons-Domain-Whitelist hinzugefügt wird. Die Domain-Whitelist ist eine Liste von Domains, die Wikimedia Commons vor dem Abrufen von Mediendateien gegenprüft. Falls deine Mediendateidomain nicht auf dieser Liste ist, wird Wikimedia Commons keine Mediendateien von dieser Domain herunterladen. Das beste Beispiel zum Einreichen deiner Anfrage ist ein tatsächlicher Link zu einer Mediendatei.',
	'gwtoolset-step-1-instructions-3-heading' => 'Domain-Whitelist',
...
[php.net/manual/en/datetime.formats.relative.php PHP manual] را برای چگونگی تنظیم آن به درستی مشاهده کنید.',
	...
	'gwtoolset-disk-write-failure' => 'کارساز نتوانست پوشه را به یک سامانهٔ پرونده بنویسد.',
...
$1',
	...
	'gwtoolset-menu-1' => 'Metatietojen määrittely',
	'gwtoolset-technical-error' => 'Tapahtui tekninen virhe.',
	'gwtoolset-required-field' => 'pakollinen kenttä',
	...
	'gwtoolset-ensure-well-formed-xml' => 'Varmista $1in avulla, että XML-tiedosto on oikein muotoiltu.',
...
$2',
	...
	'gwtoolset-step-1-heading' => 'Vaihe 1: Metatietojen tunnistaminen',
	...
	'gwtoolset-which-mediawiki-template' => 'Valitse MediaWiki-malline:',
	'gwtoolset-which-metadata-mapping' => 'Aikaisempi metatietomääritys:',
	...
	'gwtoolset-results' => 'Tulokset',
	'gwtoolset-step-2-heading' => 'Vaihe 2: Metatietojen määritteleminen',
...
$2',
	...
	'gwtoolset-step-1-instructions-3' => 'Si votre domaine de fichier média n’est pas listé ci-dessous, veuillez [$1 demander] que votre domaine de fichier média soit ajouté à la liste blanche. La liste blanche des domaines est une liste de domaines que Wikimédia Communs vérifie avant d’analyser les fichiers média. Si votre domaine de fichier média n’est pas dans cette liste, Wikimédia Communs ne téléchargera pas de fichiers média de ce domaine. Le meilleur exemple, pour envoyer votre demande, est un lien réel vers le fichier média.',
...
$1',
	...
	'gwtoolset-file-url-invalid' => 'ファイルがウィキ内にまだ存在しません。フォームでファイル URL 参照を使用したい場合は、まず、あなたのコンピューターからファイルをアップロードする必要があります。',
...
$2',
	...
	'gwtoolset-step-1-instructions-3' => '만약 미디어 파일 도메인이 아래 목록에 없다면 당신의 미디어 파일 도메인이 위키미디어 공용 화이트리스트에 추가될 수 있도록 [$1 요청]해주세요. 미디어파일 도메인이 해당 목록에 없다면 위키미디어 공용은 해당 도메인에서 미디어 파일을 내려받을 것입니다. 당신의 요청을 제출하는 최고의 예시가 미디어파일에 연결할 실제 링크입니다.',
...
* Adakah fail, XML itu dibentuk dengan sempurna? Cuba $1 ini.', # Fuzzy
	...
	'gwtoolset-step-1-instructions-2' => 'Dalam langkah ini, anda memuat naik fail metadata baru ke dalam wiki. Alatan akan cuba mengeluarkan medan-medan metadata yang terdapat dalam fail metadata itu, yang anda akan kemudian petakan pada templat Mediawiki dalam "{{int:gwtoolset-step-2-heading}}".', # Fuzzy
	'gwtoolset-step-1-instructions-3' => 'Jika domain fail media anda tidak tersenarai di bawah, sila [$1 memohon] supaya domain fail media anda ditambahkan ke dalam senarai putih domain Wikimedia Commons. Senarai putih domain merupakan senarai domain yang disemak oleh Wikimedia Commons sebelum menerima fail media. Jika domain fail media anda tidak tersenarai, Wikimedia Commons tidak akan memuat turun fail media dari domain itu. Contoh yang terbaik untuk disertakan dalam penyerahan permohonan anda adalah pautan sebenar kepada fail media.',
	...
	'gwtoolset-mapping-media-file-url-bad' => 'URL fail media tidak dapat dinilai. URL menyampaikan kandungan secara yang belum dapat dikelola oleh sambungan ini. URL yang diberikan ialah "$1".', # Fuzzy
...
$1',
	...
	'gwtoolset-file-url-invalid' => 'Filen finns ännu inte på wikin. Du måste först ladda upp filen från din dator om du vill använda filens URL-referens i formuläret.',
...
Kontrollera att <code>$wgEnableAPI</code> är inställd på <code>true</code>.',
	'gwtoolset-verify-api-writeable' => '$1-tillägget kräver att wikins API kan utföra skriv-åtgärder för behöriga användare.

Kontrollera att <code>$wgEnableWriteAPI</code> har satts till <code>true</code>.',
...
$2',
	...
	'gwtoolset-step-1-instructions-3' => 'Якщо ваш мультимедійний файл відсутній у списку нижче, то перевірте [$1 request] чи домен вашого файлу доданий у білий список доменів Вікісховища. Білий список доменів - це перелік доменів, який Вікісховище читає перед отриманням медіафайлів. Якщо домен вашого медіафайлу відсутній у списку, то Вікісховище не завантажуватиме жодного файлу з нього. Найкращим зразком перевірки дії вашого запиту є фактичне посилання на медіафайл.',
...
$1',
	...
	'gwtoolset-no-file-url' => '没有<code>file_url</code>提供解析。',
...
&lt;?xml version="1.0" encoding="UTF-8"?>',
	...
	'gwtoolset-step-1-instructions-3' => '如果您的媒体文件域未在下方列出，请[$1 在此]申请在维基共享资源域白名单中加入您的媒体文件域。域白名单是维基共享资源检查反例的媒体文件的例外列表。如果您的媒体文件域未在此列表上，维基共享资源将不会在此域下载媒体文件。最好的提交您申请的例子是实际连接一个媒体文件。',
	...
	'gwtoolset-specific-categories' => '项具体分类',
...
这些设置低于wiki的<code>$wgMaxUploadSize</code>，可在“$3”设置。请适当调整<code>php.ini</code>的设置。',
	...
	'gwtoolset-verify-api-enabled' => '$1拓展要求wiki API被启用。请确保<code>$wgEnableAPI</code>设置是<code>true</code>。',
	'gwtoolset-verify-api-writeable' => '$1拓展要求wiki API对获权用户能够执行写入动作。请确保<code>$wgEnableWriteAPI</code>设置是<code>true</code>。',
